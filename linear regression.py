...
for path in paths:
    ...
    try:
        ...
        if target_column not in df.columns:
            print("Target column '{}' not found in dataset for".format(target_column), path)
            continue
        ...
        df['Predicted_' + target_column] = model.predict(X)

        # Print the dataset after linear regression
        print("Dataset after linear regression:")
        print(df)
        print()
